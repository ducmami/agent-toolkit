...
## Mermaid Syntax Mistakes

These mistakes produce invalid or misleading Mermaid C4 output. See [c4-syntax.md](c4-syntax.md) for correct syntax.

### 1. Using Directional Relationships for Layout

**The Problem:**
Mermaid [documents](https://mermaid.js.org/syntax/c4) that shape position is adjusted by **declaration order**, not layout algorithms. `Rel_U`, `Rel_D`, `Rel_L`, `Rel_R` are **supported syntax** (PlantUML compat) but only affect arrow rendering — they do **not** move boxes.

`Lay_U`, `Lay_D`, `Lay_L`, `Lay_R` are **currently unimplemented** in Mermaid — listed in the docs but not supported.

**Wrong — using `Rel_U` expecting layout change:**
```
Rel_U(user, api, "Uses")     # api does NOT move above user
Lay_D(api, db)               # unimplemented — no effect
```

**Correct — declaration order controls placement:**
```mermaid
C4Context
...
Want a box higher? **Declare it earlier.** Use plain `Rel`, not `Rel_U`.

### 2. Wrong Style Function or Parameter Names

**The Problem:**
- `UpdateRelStyle`: `textColor`, `lineColor`, `offsetX`, `offsetY` (positional order)
- `UpdateElementStyle`: `bgColor`, `fontColor`, `borderColor`, `shadowing`, `shape`
- Lowercase `updateElementStyle` is a Mermaid quirk — may style relationships instead of elements

**Wrong:**
```
updateElementStyle(api, $bgColor="grey")          # wrong case; unpredictable
UpdateRelStyle(spa, api, $bgColor="blue")       # bgColor is not a Rel param
UpdateRelStyle(spa, api, "blue", "", "", "-30")   # fragile empty placeholders
```
...
### 3. Generating Code Without Complete Entity List

**The Problem:**
Mermaid C4 requires boundaries declared before nested elements, elements before relationships, and declaration order for layout. Writing code incrementally with missing services produces patchwork diagrams.

**Wrong approach:**
Start writing Mermaid immediately when the user says "we have a frontend and some microservices."

**Correct approach:**
1. Draft all entities in a table:

| Alias | Type | Label | Technology |
|-------|------|-------|------------|
| user | Person | Customer | — |
| gateway | Container | API Gateway | Kong |
| orderSvc | Container | Order Service | Node.js |

2. Ask user to confirm or fill gaps
3. Then write Mermaid with complete structure
...
- [ ] All entities confirmed before Mermaid code was written
- [ ] Style calls (`UpdateLayoutConfig`, `UpdateElementStyle`, `UpdateRelStyle`) at diagram end
- [ ] PascalCase `UpdateElementStyle` / `UpdateRelStyle` — never lowercase `updateElementStyle`
- [ ] `UpdateRelStyle` uses `$textColor`/`$lineColor`/`$offsetX`/`$offsetY` — not `bgColor`
- [ ] No `Lay_*` directives; layout via declaration order + `UpdateLayoutConfig`
- [ ] `Rel` only (not `BiRel`/`Rel_U`/`Rel_D`) unless user explicitly requests PlantUML-compat syntax
