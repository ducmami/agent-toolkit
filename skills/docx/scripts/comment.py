...
def _append_xml(xml_path: Path, root_tag: str, content: str) -> None:
    ...
    elements = dom.getElementsByTagName(root_tag)
    if not elements:
        raise ValueError(f"Tag '{root_tag}' not found in {xml_path}")
    root = elements[0]
...
def add_comment(
    ...
    """Returns (success, para_id_or_empty, message)."""
    word = Path(unpacked_dir) / "word"
    if not word.exists():
        return False, "", f"Error: {word} not found"
    ...
    if parent_id is not None:
        ...
        if not parent_para:
            return False, "", f"Error: Parent comment {parent_id} not found"
        _append_xml(
            ext,
            "w15:commentsEx",
    ...
    return True, para_id, f"Added {action} {comment_id} (para_id={para_id})"
...
if __name__ == "__main__":
    ...
    success, para_id, msg = add_comment(
        args.unpacked_dir,
        args.comment_id,
        args.text,
        args.author,
